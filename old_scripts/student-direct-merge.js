...
async function main() {
    try {
        const result = await processStudentData();
        console.log(`\nOutput files created:`);
        console.log(`- ${result.outputFilename} (${result.processedCount} records)`);
        console.log(`- ${result.incompleteFilename} (${result.incompleteCount} records)`);
    } catch (error) {
        console.error('\nScript failed:', error);
        process.exit(1);
    }
