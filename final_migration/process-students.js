...
// Function to update main_school_branch_id based on branch_id lists
function updateMainSchoolBranchId(record) {
    const branchId = record.branch_id;
    // Only update if branch_id exists and main_school_branch_id is empty
    if (branchId && (!record.main_school_branch_id || record.main_school_branch_id === '')) {
        // Check if branch_id is in main_school_takahagi list
        if (main_school_takahagi.includes(branchId)) {
            record.main_school_branch_id = '008882';
        }
        // Check if branch_id is in main_school_yabu list
        else if (main_school_yabu.includes(branchId)) {
            record.main_school_branch_id = '008885';
        }
    }
    return record;
}
...
async function processStudentData(inputFile) {
    try {
        ...
        const studentInfoData = await readAndParseCSV('./batch1/student_info.csv');
        ...
        const customerData = await readAndParseCSV('./batch1/customer.csv');
        ...
        const studentHistoryData = await readAndParseCSV('./batch1/student_info_history.csv');
        ...
        const outputFilename = './output/processed_student_data.csv';
        const incompleteFilename = './output/processed_student_data_incomplete.csv';
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            // Update main_school_branch_id based on branch_id lists as final step
            processedChunk.forEach(record => {
                updateMainSchoolBranchId(record);
            });
            incompleteChunk.forEach(record => {
                updateMainSchoolBranchId(record);
            });
...
async function main() {
    ...
    try {
        ...
        const inputFile = inputFileArg || './batch1/student.csv';
